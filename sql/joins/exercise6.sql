...
SELECT 
    CONCAT(ft.firstname, ' ', ft.surname) AS member, 
    facs.name AS facility, 
    CASE 
        WHEN ft.memid = 0 THEN
...
FROM
    cd.members ft
INNER JOIN 
    cd.bookings bks ON ft.memid = bks.memid
INNER JOIN 
    cd.facilities facs ON bks.facid = facs.facid
WHERE
    bks.starttime >= '2012-09-14' AND 
    bks.starttime < '2012-09-15' AND (
        (ft.memid = 0 AND bks.slots * facs.guestcost > 30) OR
        (ft.memid != 0 AND bks.slots * facs.membercost > 30)
    )
ORDER BY 
    cost DESC;