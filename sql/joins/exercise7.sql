...
-- подход с || не работает здесь, но работает на сайте. Здесь если при использовани || в строках значения 0 и null
SELECT 
    CONCAT(mems.firstname, ' ', mems.surname) AS member,
    COALESCE(
        (SELECT 
            CONCAT(recs.firstname, ' ', recs.surname) 
         FROM 
            cd.members recs
         WHERE 
            recs.memid = mems.recommendedby
        ), 'No recommender') AS recommender
FROM 
    cd.members mems
ORDER BY 
    member;